...
image = cv2.imread('picture/image4.png', cv2.IMREAD_GRAYSCALE)
height, width= image.shape

# image = cv2.medianBlur(image, 7)
# _, image = cv2.threshold(image, 128, 255, cv2.THRESH_BINARY_INV)
_, image = cv2.threshold(image, 128, 255, cv2.THRESH_TOZERO_INV)

# image = cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 7, 2) #adaptive threshold gaussian filter used

reader = easyocr.Reader(['en'], gpu=True)

# 1336
score1 = image[int(40 * height / 1440):int(75 * height / 1440),int(1225 * width / 2560):int(1280 * width / 2560)]
score2 = image[int(40 * height / 1440):int(75 * height / 1440),int(1285 * width / 2560):int(1330 * width / 2560)]
...
# contours, _ = cv2.findContours(score1, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
# contour_mask = np.zeros_like(score1)

# cv2.drawContours(contour_mask, contours, -1, 255, thickness=cv2.FILLED)

# score1 = cv2.cvtColor(score1, cv2.COLOR_GRAY2BGR)
# score1[contour_mask == 255] = [0, 255, 0]  # Set contour pixels to green
