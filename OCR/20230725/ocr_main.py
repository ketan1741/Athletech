...
def process_frame(frame_queue):
    ...
    while True:
        ...
        data_dict[str(ctime)] = {}
        # ----------------------------------------------------------score----------------------------------------------------------#
        # scores = image[int(40 * height / 1440):int(75 * height / 1440),int(1222 * width / 2560):int(1336 * width / 2560)]
        # kernel = np.array([[-1,-1,-1], [-1,9,-1], [-1,-1,-1]])
        # scores = cv2.filter2D(scores, -1, kernel)
        # results = reader.readtext(scores.astype(np.uint8), allowlist='0123456789', paragraph=True)
        ...
        result_cs = cv2.matchTemplate(winner, cs_icon, cv2.TM_CCOEFF_NORMED)
        result_ss = cv2.matchTemplate(winner, ss_icon, cv2.TM_CCOEFF_NORMED)
        locations_cs = np.where(result_cs >= threshold)
        locations_ss = np.where(result_ss >= threshold)
        print('time: {}, alive: {}'.format(ctime,player_alive))
        if locations_cs[0].size != 0:
            cs_score += 1
            player_alive = True
        elif locations_ss[0].size != 0:
            ss_score += 1
            player_alive = True
        else:
            pass
    ...
    # ----------------------------------------------------------Kill----------------------------------------------------------#
            ...
            # data_dict[str(ctime)]['score'] = 'None'
            # data_dict[str(ctime)]['HP_AC'] = 'None'
