...
def process_frame(frame_queue):
    ...
    while True:
        ...
        data_dict[str(ctime)] = {}
        # ----------------------------------------------------------score----------------------------------------------------------#
        data_dict[str(ctime)]['player_get_killed'] = kill_flg
        kill_flg = 0
        ...
        if old_round:
            ...
            if len(winner_pr) >= 12 and index_te == 40: # true winner
                old_round = False
                distances = [Levenshtein.distance(winner_pr, target) for target in winner_list]
                min_distance_index = distances.index(min(distances))
                index_te = 0
                data_dict[str(ctime)]['distance'] = distances
                data_dict[str(ctime)]['index'] = min_distance_index
